...
from app.models import User, UserRole as ModelRole, Employment, Bonus
...
def calc_current_month_salary(db: Session, user_id: int, month_start: date, month_end: date) -> float:
    ...
    bonuses_sum_dec = db.scalar(
        select(func.coalesce(func.sum(Bonus.amount), 0)).where(
            and_(Bonus.user_id == user_id, Bonus.bonus_date >= month_start, Bonus.bonus_date <= month_end)
        )
    ) or decimal.Decimal("0.00")
    bonuses = float(bonuses_sum_dec)

    return round(base + bonuses, 2)

def gen_pdf_bytes(*, full_name: str, employee_code: str, cnp: str, month_label: str, salary_value: float) -> bytes:
    buf = io.BytesIO()
    ...
    lines = [
        ...
        f"CNP: {cnp}",
        f"Month: {month_label}",
        f"Salary to be paid: {salary_value:.2f} RON",
    ]
    for line in lines:
...
def create_pdfs_for_employees(
    ...
    for emp in employees:
        salary = calc_current_month_salary(db, emp.id, mstart, mend)
